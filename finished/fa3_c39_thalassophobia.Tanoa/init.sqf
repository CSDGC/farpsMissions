...
f_var_viewDistance_default = 1000;
f_var_viewDistance_tank = 1000;
f_var_viewDistance_car = 1000;
f_var_viewDistance_rotaryWing = 2000;
f_var_viewDistance_fixedWing = 2000;
...
if (!isDedicated) then {
	// Display a short text intro
 	[] spawn {
	 waitUntil {time > 15};
		 ["Thalassophobia","Somewhere around the Horizon Islands"] call ws_fnc_showIntro;
	};// Display a short text intro
};
//------------------------------------------------------------------------------
//remove 40mm GMG and replace it with something a bit weaker
_boats = [VehNATO_AV,VehNATO_BV,VehNATO_CV];
{
  _x removeWeaponTurret ["GMG_40mm",[0]];
  _x addWeaponTurret ["HMG_NSVT",[0]]; //12.7x108 HMG
  _x addMagazineTurret ["450Rnd_127x108_Ball", [0]];
} forEach _boats;
//------------------------------------------------------------------------------
