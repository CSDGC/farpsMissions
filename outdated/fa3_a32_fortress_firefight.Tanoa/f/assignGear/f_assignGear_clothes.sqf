...
// Select helmet (COD simulator gets driver helmet because then it takes 2 headshots to kill no matter where on the head you hit)
_helmet = switch (f_param_loadouts) do {
	case 0:{_baseHelmet};
	case 1:{_baseHelmet};
	case 2:{_heavyHelmet};
	case 3:{_baseHelmet};
...
// uniform needs selecting for sniper loadout
_uniform = switch (f_param_loadouts) do {
	case 5:{_Uniform};
